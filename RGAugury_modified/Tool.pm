  ...
  foreach my $file (@split_files) {
      if (-e $file) {
          ...
          if ($pid =fork){
              ...
              #child process here
              #print STDERR "222222 $splited_cmd\n";
              exec($splited_cmd);
          }
...
sub blastp_parallel {
  ...
  $cmd .= "-out {out} -num_threads 2 -outfmt '6 qseqid sseqid pident length mismatch ";
...
sub pfamscan_parallel {
    ...
    my $cmd = "pfam_scan.pl -outfile {out} -e_seq $e_seq -e_dom $e_dom --cpu 1 -as -fasta {in} -dir     $pfam_index_folder";
